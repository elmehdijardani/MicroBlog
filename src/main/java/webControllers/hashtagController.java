...
public class hashtagController {
	...
	String getLatestTweetsByHashtag(@RequestParam String tag, Model model) {
		List<LinkedHashMap<String, String>> tweets = restTemplate.getForObject("http://localhost:8080/tweets", List.class);
		Map<String, String> paramh = new HashMap<>();
		paramh.put("tag", tag);
		List<LinkedHashMap<String, String>> hashtags = restTemplate.getForObject("http://localhost:8080/hashtags?tag={tag}", List.class, paramh);

		List<LinkedHashMap<String, String>> tweetsCode = new ArrayList<>();
		
		for (LinkedHashMap<String, String> htag : hashtags) {
			for (LinkedHashMap<String, String> tweet : tweets) {
				if(String.valueOf(htag.get("tweetid")).equals(String.valueOf(tweet.get("id")))){
